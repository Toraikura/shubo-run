...
## v2.0：スワイプでつないで、フィーバーへ

- **iPhone縦持ち**：プレイ中は盤面・方向ボタン・スキルを画面内に固定。スワイプでも曲がれます。指を離しても進み、■で停止。曲がれない方向は次の角まで予約。
- **10連続でフィーバー**：コンボ中に糖・大粒の糖を10個集めると4秒間、採取点×2＋接触無効。緑の粒はコンボを維持しますが充填しません。発動中は延長せず、終了後に改めて貯めます。
- **ステージ間の選択**：通常のエネルギー+25に加え、6秒保護／エネルギー100／制限時間+12秒のどれかを選びます。
- **今日のコース**：日本時間で日替わり。同じ日・設定は同じ配置。定番260907は旧版の道を保持し、別シードは全通路のつながりを確認した異なる迷路になります。
- **音の手応え**：初期OFF。♪ボタンをタップすると採取・コンボ・フィーバー・スキル・クリア音が鳴ります。合成音なので外部素材の取得はありません。
- **記録を保持**：旧版のハイスコアをそのまま読み込みます。音・日替わり設定は別キーに保存。コースや旧版の係数を混ぜた自己記録なので、公平なオンラインランキングではありません。

iPhone実機Safariは未検証。ブラウザでのスマホ幅とWebKitエンジンの確認を、実機試験と混同しません。最新の結果は[リリース検証](docs/RELEASE_V2.md)。
...
2. 盤面スワイプ、画面下の方向ボタン、または矢印 / WASDで移動。押すと進み続け、曲がれない方向の入力は次に曲がれる場所まで保持。■ / Xで停止。
